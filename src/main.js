...
document.querySelector("#app").innerHTML = `
  <main class="app-shell">
    <header class="app-header">
      ...
      <div class="version-pill">v0.21 stable Buttery Fuzz checkpoint</div>
    </header>
    ...
    <section class="panel patch-summary">
      ...
      <p id="patchSummaryText">Stable Buttery Fuzz checkpoint. The tested v0.20 Buttery Fuzz state is frozen. No sound engine running. Press Start Oscillator or Start Noise to test one quiet source. Output is set to 70%, clamped to a safe maximum. Low-pass cutoff is set to 2600 Hz and shapes both before and after the fuzz stage. Resonance reaches 40 for a strong audible peak. Buttery Fuzz is set to 70% and uses rounded saturation instead of hard clipping. A stronger left/right stereo-width branch is active after the fuzz mix. Feedback is not connected in v0.21. The spectral meters listen after the master Output control. The spectral faders are visual only. No fake self-oscillation is connected.</p>
    </section>
  </main>
`;
...
function updatePatchSummary() {
  ...
  const checkpointText = "The tested v0.20 Buttery Fuzz state is frozen in this v0.21 checkpoint.";
  ...
  if (wasPanicStopped && !isOscillatorRunning && !isNoiseRunning) {
    patchSummaryText.textContent =
      `Panic Stop used. Oscillator and noise are stopped, and output has been silenced. ${checkpointText} The analyser meters will fall as the output reaches silence. Press Start Oscillator or Start Noise to resume normal use. ${fuzzSummary} ${safetyText} Low-pass cutoff is set to ${cutoffFrequency} Hz. Resonance is set to ${resonanceAmount}. ${notYetText}`;
    return;
  }

  if (isOscillatorRunning && isNoiseRunning) {
    patchSummaryText.textContent =
      `One quiet sawtooth oscillator and one quiet white noise source are running through one low-pass filter set to ${cutoffFrequency} Hz with resonance set to ${resonanceAmount}, then through the frozen Buttery Fuzz stage, then through a post-fuzz low-pass filter that follows Cutoff, then through the master Output control set to ${outputPercent}%. ${checkpointText} ${fuzzSummary} ${safetyText} ${spectralText} ${notYetText}`;
    return;
  }
  ...
  patchSummaryText.textContent =
    `Stable Buttery Fuzz checkpoint. ${checkpointText} No sound engine running. Press Start Oscillator or Start Noise to test one quiet source. Output is set to ${outputPercent}%. ${fuzzSummary} ${safetyText} Low-pass cutoff is set to ${cutoffFrequency} Hz. Resonance is set to ${resonanceAmount}. ${spectralText} ${notYetText}`;
}
